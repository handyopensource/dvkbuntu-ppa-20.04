...
PlasmaCore.ColorScope {
    ...
    MouseArea {
        anchors.fill: parent
        onClicked: {
            qprocess.launch('createWaveFromItem "Stop"');
    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            LogoutButton {
                ...
                MouseArea {
                    ...
                    anchors.fill: parent;
                    onEntered: {
                        qprocess.launch('createWaveFromItem "' + suspendButton.text + '"');
                        suspendButton.opacity = 1;
                    }
                    onExited: {
                        qprocess.launch('createWaveFromItem "Stop"');
                        suspendButton.opacity = 0.5;
                    }
                    onClicked: suspendButton.action();
                }
            }
            LogoutButton {
                id: hibernateButton
                ...
                MouseArea {
                    ...
                    anchors.fill: parent;
                    onEntered: {
                        qprocess.launch('createWaveFromItem "' + hibernateButton.text + '"');
                        hibernateButton.opacity = 1;
                    }
                    onExited: {
                        qprocess.launch('createWaveFromItem "Stop"');
                        hibernateButton.opacity = 0.5;
                    }
                    onClicked: hibernateButton.action();
                }
            }
            LogoutButton {
                id: rebootButton
                ...
                MouseArea {
                    ...
                    anchors.fill: parent;
                    onEntered: {
                        qprocess.launch('createWaveFromItem "' + rebootButton.text + '"');
                        rebootButton.opacity = 1;
                    }
                    onExited: {
                        qprocess.launch('createWaveFromItem "Stop"');
                        rebootButton.opacity = 0.5;
                    }
                    onClicked: rebootButton.action();
                }
            }
            LogoutButton {
                id: shutdownButton
                ...
                MouseArea {
                    ...
                    anchors.fill: parent;
                    onEntered: {
                        qprocess.launch('createWaveFromItem "' + shutdownButton.text + '"');
                        shutdownButton.opacity = 1;
                    }
                    onExited: {
                        qprocess.launch('createWaveFromItem "Stop"');
                        shutdownButton.opacity = 0.5;
                    }
                    onClicked: shutdownButton.action();
                }
            }
            LogoutButton {
                id: logoutButton
                ...
                MouseArea {
                    ...
                    anchors.fill: parent;
                    onEntered: {
                        qprocess.launch('createWaveFromItem "' + logoutButton.text + '"');
        ...
        PlasmaComponents.Label {
            ...
            text: {
                switch (sdtype) {
                    case ShutdownType.ShutdownTypeReboot:
                        if (root.remainingTime == 30 || root.remainingTime == 25 || root.remainingTime == 20 || root.remainingTime == 15 || root.remainingTime == 10 || root.remainingTime == 5) {
                            qprocess.launch('createWaveFromItem "' + i18ndp("plasma_lookandfeel_org.kde.lookandfeel", "Restarting in 1 second", "Restarting in %1 seconds", root.remainingTime) + '"');
                        }
                        return i18ndp("plasma_lookandfeel_org.kde.lookandfeel", "Restarting in 1 second", "Restarting in %1 seconds", root.remainingTime);
                    case ShutdownType.ShutdownTypeHalt:
                        if (root.remainingTime == 30 || root.remainingTime == 25 || root.remainingTime == 20 || root.remainingTime == 15 || root.remainingTime == 10 || root.remainingTime == 5) {
                            qprocess.launch('createWaveFromItem "' + i18ndp("plasma_lookandfeel_org.kde.lookandfeel", "Shutting down in 1 second", "Shutting down in %1 seconds", root.remainingTime) + '"');
                        }
                    ...
                    default:
                        if (root.remainingTime == 30 || root.remainingTime == 25 || root.remainingTime == 20 || root.remainingTime == 15 || root.remainingTime == 10 || root.remainingTime == 5) {
                            qprocess.launch('createWaveFromItem "' + i18ndp("plasma_lookandfeel_org.kde.lookandfeel", "Logging out in 1 second", "Logging out in %1 seconds", root.remainingTime) + '"');
                        }
